...
defmodule AshPostgres.Test.Post do
  ...
  attributes do
    ...
    # Base reading time for the post content
  ...
  calculations do
    ...
    # Test calculations using fragments that reference multiple aggregates
    # These should trigger binding issues when loaded at runtime together
    calculate(:comment_metric, :integer, expr(fragment("(? * 100)", count_of_comments)))

    # Complex calculation referencing multiple aggregates like zelo case
    calculate(:complex_comment_metric, :integer, expr(
      fragment("COALESCE(?, 0) + COALESCE(?, 1) * COALESCE(?, 0)",
        sum_of_comment_likes_test, count_of_comments, max_comment_likes)
    ))

    # Another complex calculation to force multiple aggregates
    calculate(:multi_agg_calc, :integer, expr(
      fragment("(? * ?) + ?", count_of_comments, count_of_high_like_comments, sum_of_comment_likes_test)
    ))
    ...
    # Complex calculation using SQL fragments with multiple filtered aggregates
    calculate :estimated_reading_time,
      ...
      public?(true)
      load [:total_edited_time, :total_planned_time, :total_comment_time]
    end
  end
  ...
  aggregates do
    ...
    # Aggregates for testing complex calculation patterns with filtered sums
    ...
    # Count aggregate with filter for testing complex interaction patterns
    ...
    # Additional aggregates for testing complex fragment binding scenarios
