...
defmodule AshPostgres.BulkUpdateAfterTransactionTest do
  @moduledoc """
  Tests for after_transaction hooks in bulk update operations with PostgreSQL.

  These tests verify the fix for after_transaction hooks running correctly
  when batch transactions fail in bulk_update with :stream strategy.

  Mirrors the :stream strategy Mnesia tests from the ash repo:
  ash/test/actions/bulk/bulk_update_after_transaction_test.exs
  """
  ...
  describe ":stream strategy" do
    ...
    test "hooks run outside batch transaction - no warning about running inside tx" do
      # Use 5 posts for consistency with other tests
      ...
      assert_receive {:postgres_after_transaction_called, _id}
      assert_receive {:postgres_after_transaction_called, _id}
      assert_receive {:postgres_after_transaction_called, _id}
      assert_receive {:postgres_after_transaction_called, _id}
      assert_receive {:postgres_after_transaction_called, _id}
      ...
      # (check for the specific warning text, not just "after_transaction" which matches table name)
    ...
    test "partial failure with transaction: :all rolls back all records" do
      ...
      # Batches (sorted by title, batch_size 2): [a_ok_1, a_ok_2], [a_ok_3, z_fail_4], [z_fail_5]
      posts =
      ...
      final_records = AfterTransactionPost |> Ash.read!()
      assert Enum.all?(final_records, &is_nil(&1.status))
    end

    test "partial failure with transaction: :batch commits first batch, rolls back second" do
      # Prefixed titles for deterministic sorting: a_ before z_
      # Batches (sorted by title, batch_size 2): [a_ok_1, a_ok_2], [z_fail_3, z_fail_4], [z_fail_5]
      posts =
      ...
      # Verify final state:
      # - a_ok_1, a_ok_2: status = "updated" (batch 1 committed)
      # - z_fail_3, z_fail_4, z_fail_5: status = nil (batch 2 rolled back, batch 3 not processed)
      final_records =
        AfterTransactionPost
        |> Ash.Query.filter(id in ^post_ids)
        |> Ash.Query.sort(:title)
      ...
      statuses = Enum.map(final_records, & &1.status)
      assert statuses == ["updated", "updated", nil, nil, nil]
    end

    test "after_transaction error with transaction: :batch - batch still commits (runs outside tx)" do
      # Prefixed titles for deterministic sorting: a_ before z_
      # Batches (sorted by title, batch_size 2): [a_ok_1, a_ok_2], [a_ok_3, z_fail_4], [z_ok_5]
      posts =
      ...
      statuses = Enum.map(final_records, & &1.status)
      assert statuses == ["updated", "updated", "updated", "updated", nil]
    end
  ...
  describe ":atomic strategy" do
    test "hooks execute on success" do
      ...
      result =
        ...
        |> Ash.Query.filter(id in ^post_ids)
        |> Ash.bulk_update(
          :atomic_update_with_after_transaction,
          %{},
          strategy: :atomic,
          return_records?: true,
          return_errors?: true
      ...
      assert Enum.all?(final_records, &(&1.status == "updated"))
    end
    ...
    test "hooks run on failure" do
      ...
      # Titles unchanged - transaction rolled back
      expected_titles = Enum.map(1..5, &"title_#{&1}") |> Enum.sort()
      actual_titles = final_records |> Enum.map(& &1.title) |> Enum.sort()
    ...
    test "after_action failure rolls back ALL records (single atomic transaction)" do
      ...
      result =
        ...
        |> Ash.Query.sort(:title)
        |> Ash.bulk_update(
          :atomic_update_with_conditional_after_action_error,
          %{},
          strategy: :atomic,
          return_errors?: true
        )
      ...
      statuses = Enum.map(final_records, & &1.status)
      assert statuses == [nil, nil, nil, nil, nil]
    end
    ...
    test "partial failure sets status to :partial_success" do
      ...
      result =
        ...
        |> Ash.Query.sort(:title)
        |> Ash.bulk_update(
          :atomic_update_with_partial_failure,
          %{},
          strategy: :atomic,
          return_errors?: true
        )
      ...
      # All records were updated - after_transaction runs OUTSIDE tx
      final_records =
        AfterTransactionPost
        |> Ash.Query.filter(id in ^post_ids)
        |> Ash.read!()
