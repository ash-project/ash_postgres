...
defmodule AshPostgres.MigrationGenerator do
  ...
  def generate(domains, opts \\ []) do
    ...
    collation_migration_files =
      create_collation_migrations(repos, opts)
    ...
    case extension_migration_files ++
           collation_migration_files ++ tenant_migration_files ++ migration_files do
      [] ->
  ...
  defp create_collation_migrations(repos, opts) do
    for repo <- repos, repo.migrate_extensions?() do
      snapshot_path = snapshot_path(opts, repo)
      repo_name = repo_name(repo)
      ...
      requested = repo.installed_collations()

      requested_names = Enum.map(requested, & &1.name)
      installed_names = Enum.map(installed, & &1.name)

      to_install = Enum.reject(requested, &(&1.name in installed_names))

      to_drop =
        installed
        |> Enum.reject(&(&1.name in requested_names))
        |> Enum.map(&decode_collation/1)

      if Enum.empty?(to_install) && Enum.empty?(to_drop) do
        []
      else
        dev = if opts.dev, do: "_dev"
        ...
        count =
          opts
          |> migration_path(repo)
          |> Path.join("*_migrate_resources_collations*")
          |> Path.wildcard()
          |> length()
          |> Kernel.+(1)
        ...
        base_name =
          if opts.name do
            "#{opts.name}_collations_#{count}"
          else
            require_name!(opts)
            "migrate_resources_collations_#{count}"
          end

        migration_name = "#{timestamp()}_#{base_name}#{dev}"
        module_name = Module.concat([repo, Migrations, Macro.camelize(base_name)])
        ...
        up =
          Enum.map_join(to_install ++ to_drop, "\n", fn collation ->
            sql =
              if collation in to_install do
                AshPostgres.CustomCollation.create_sql(collation)
              else
                AshPostgres.CustomCollation.drop_sql(collation)
              end
        ...
        defmodule #{inspect(module_name)} do
          @moduledoc \"\"\"
          Creates any collations that are mentioned in the repo's `installed_collations/0` callback
          ...
          def down do
            # Uncomment this if you actually want to reverse the collation changes
            # when this migration is rolled back:
            #{down}
          end
        end
  ...
  defp encode_collation(%AshPostgres.CustomCollation{} = collation) do
    collation
    |> Map.from_struct()
  ...
  defp decode_collation(map) do
    ...
    struct(AshPostgres.CustomCollation, Map.put(map, :provider, provider))
  end
