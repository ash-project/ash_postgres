defmodule AshPostgres.MigrationGenerator do
  ...
  def generate(apis, opts \\ []) do
    ...
    # TODO: Remove after stable release (2.0)
    create_index_renaming_migrations(tenant_snapshots, opts, true)
    create_index_renaming_migrations(snapshots, opts, false)
  ...
  defp create_index_renaming_migrations(snapshots, opts, tenant?) do
    identities =
      snapshots
      |> Enum.map(&get_existing_snapshot(&1, opts))
      |> Enum.reject(&Map.has_key?(&1, :ash_version))
      |> Enum.map(fn snapshot ->
        snapshot =
          snapshot
          |> Map.drop([:hash])
          |> Map.put(:ash_version, vsn(:ash))
          |> Map.put(:ash_postgres_version, vsn(:ash_postgres))
      ...
      |> Enum.map(fn {repo, snapshots} ->
        identity_names =
          snapshots
          |> Enum.reject(&(&1.identities == []))
          |> Enum.map(fn snapshot ->
            Enum.map(snapshot.identities, &{snapshot.table, &1.name})
          end)
          |> List.flatten()
    ...
    identities =
      Enum.reject(identities, fn {_repo, identity_names} ->
        identity_names == []
      end)

    for {repo, identity_names} <- identities do
      migration_name = "#{timestamp(true)}_rename_indexes"

      migration_file =
        migration_path(opts, repo, tenant?)
        |> Path.join(migration_name <> ".exs")

      module_name =
        if tenant? do
          Module.concat([repo, TenantMigrations, "RenameIndexes"])
        else
          Module.concat([repo, Migrations, "RenameIndexes"])
        end

      up =
        Enum.map(identity_names, fn {table, identity_name} ->
          "execute(\"ALTER INDEX #{table}_#{identity_name}_unique_index " <>
            "RENAME TO #{table}_#{identity_name}_index\")\n"
        end)

      down =
        Enum.map(identity_names, fn {table, identity_name} ->
          "execute(\"ALTER INDEX #{table}_#{identity_name}_index " <>
            "RENAME TO #{table}_#{identity_name}_unique_index\")\n"
        end)

      contents = """
      defmodule #{inspect(module_name)} do
        @moduledoc \"\"\"
        AshPostgres version 0.38.0 changes the naming schema of indexes.
  ...
  defp do_snapshot(resource, table \\ nil) do
    snapshot = %{
      ash_version: vsn(:ash),
      ash_postgres_version: vsn(:ash_postgres),
  ...
  defp vsn(dependency) do
    Application.spec(dependency, :vsn) |> to_string()
  end
